    ...
    <div className="modal">
      ...
      <form className="modal__content form" onSubmit={submit}>
        ...
        {activeAction === 'openAccount' ? (
          ...
          <>{accountSelect}<label>Сумма<input type="number" min="1" step="0.01" value={payload.amount ?? ''} onChange={(event) => update('amount', event.target.value)} placeholder="Введите сумму" disabled={submitting} /></label></>
        )}
        {activeAction === 'transfer' && (
          <>
            ...
            {isExternalTransfer ? (
              <>
                <label>Номер счёта получателя<input value={payload.toAccountNumber ?? ''} onChange={(event) => update('toAccountNumber', event.target.value)} placeholder="Введите номер счёта" disabled={submitting} /></label>
