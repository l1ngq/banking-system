      ...
      <form className="modal__content form" onSubmit={submit}>
        ...
        {activeAction === 'transfer' && (
          <>
            <div className="transfer-mode">
              <button
                type="button"
                className={payload.transferMode === 'own' ? 'button-primary' : 'text-button'}
                onClick={() => update('transferMode', 'own')}
                disabled={submitting}
              >
              ...
              <button
                type="button"
                className={payload.transferMode === 'external' ? 'button-primary' : 'text-button'}
                onClick={() => update('transferMode', 'external')}
                disabled={submitting}
              >
                По номеру счёта
              </button>
            ...
            {isExternalTransfer ? (
              <>
                <label>
                  ...
                  <input
                    value={payload.toAccountNumber ?? ''}
                    onChange={(event) => update('toAccountNumber', event.target.value)}
                    placeholder="40817810000000000014"
                    inputMode="numeric"
                    disabled={submitting}
                  />
                </label>
                ...
                <p className="form__hint">
                  Перевод по номеру счёта пока работает как черновик. Заявка появится в истории,
                  но деньги не будут списаны до реализации этой функции в backend.
                </p>
              </>
              ...
              <p className="form__hint">
                Откройте второй счёт, чтобы выполнить перевод между своими счетами.
              </p>
            )}
          </>
        )}
        ...
        {activeAction === 'exchange' && (
          <label>
            Получить валюту
            <select
              value={(payload.toCurrency as CurrencyCode) ?? 'USD'}
              onChange={(event) => update('toCurrency', event.target.value)}
              disabled={submitting}
            >
              <option value="USD" disabled={selectedAccount?.currency === 'USD'}>
                USD
              </option>
              <option value="EUR" disabled={selectedAccount?.currency === 'EUR'}>
                EUR
              </option>
              <option value="RUB" disabled={selectedAccount?.currency === 'RUB'}>
                RUB
              </option>
            </select>
          </label>
